/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DigitalLibraryWhereInput } from "./DigitalLibraryWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DigitalLibraryOrderByInput } from "./DigitalLibraryOrderByInput";

@ArgsType()
class DigitalLibraryFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DigitalLibraryWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DigitalLibraryWhereInput, { nullable: true })
  @Type(() => DigitalLibraryWhereInput)
  where?: DigitalLibraryWhereInput;

  @ApiProperty({
    required: false,
    type: [DigitalLibraryOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DigitalLibraryOrderByInput], { nullable: true })
  @Type(() => DigitalLibraryOrderByInput)
  orderBy?: Array<DigitalLibraryOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DigitalLibraryFindManyArgs as DigitalLibraryFindManyArgs };
